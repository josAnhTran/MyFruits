...
router.get(
  "/search-many",
  validateSchema(search_deleteManyCustomersSchema),
...
router.post(
  "/insert-many",
  validateSchema(insertManyCustomersSchema),
  function (req, res, next) {
    ...
    listBirthdays.map((customer) => {
      if (customer.birthday) {
        customer.birthday = new Date(
          moment(customer.birthday).utc().local().format("YYYY-MM-DD")
        );
      }
    });
...
router.patch(
  "/update-many",
  validateSchema(updateManyCustomersSchema),
  function (req, res, next) {
    const query = req.query;
  ...
  switch (key) {
    ...
    default:
      res
        .status(404)
        .json({
          message: "Searching function is fail",
          solution: "Please check your formatting request",
        });
      return;
  }
  findDocuments({ query: query }, COLLECTION_NAME)
